...
evacdf = df[df['Tweet'].str.contains("evacuat", na = False)]
evacdf['chunks'] = ""
evacdf = evacdf.reset_index()
...
for index, row in evacdf.iterrows():
    evacdf.at[index, 'chunks'] = get_area(row['Tweet'], 'GPE')


evacdf.to_sql("evacuationTweets", sql, if_exists = "replace")