...
sheltdf = df[df['Tweet'].str.contains("medic|injur|sick", na = False)==True]
sheltdf['chunks'] = ""
sheltdf = sheltdf.reset_index()
...
for index, row in sheltdf.iterrows():
    sheltdf.at[index, 'chunks'] = get_area(row['Tweet'], 'GPE')


sheltdf.to_sql("medTweets", sql, if_exists = "replace")